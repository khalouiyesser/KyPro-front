...
import {
    Building2, Plus, Search, Filter, MoreVertical,
    Power, Trash2, Eye, ChevronDown, AlertTriangle, X,
} from 'lucide-react';
import toast from 'react-hot-toast';
import {adminCompaniesApi} from "../../api/adminCompaniesApi";
// import { adminCompaniesApi } from '../../api/adminApi'; // ← import correct
...
const AdminCompaniesPage: React.FC = () => {
    ...
    const [filterStatus, setFilterStatus] = useState<string[]>([]);
    const [showFilters,  setShowFilters]  = useState(false);
    const [confirmModal, setConfirmModal] = useState<{ type: 'toggle' | 'delete'; company: Company } | null>(null);

    // ── Fetch — appelle /admin/companies ──────────────────────────────────────
    const { data, isLoading } = useQuery({
        queryKey: ['admin-companies'],
        queryFn:  () => adminCompaniesApi.getAll(),   // ← correct endpoint
    });

    // L'API renvoie { companies, total, page, limit, pages }
    ...
    const toggleMutation = useMutation({
        mutationFn: (c: Company) =>
            c.isActive
                ? adminCompaniesApi.suspend(c._id, 'Suspendu manuellement')
                : adminCompaniesApi.reactivate(c._id),
        onSuccess: (_, c) => {
            toast.success(`Entreprise ${c.isActive ? 'suspendue' : 'activée'}`);
            queryClient.invalidateQueries({ queryKey: ['admin-companies'] });
        },
                            ...
                            {filtered.map(company => {
                                ...
                                return (
                                    <tr key={company._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-0.5 rounded-md text-xs font-semibold ${plan.cls}`}>{plan.label}</span>
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1.5">
                                                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${status.dot}`} />
